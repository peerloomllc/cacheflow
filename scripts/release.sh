#!/usr/bin/env bash
# Local release script for Cache Flow (a single-file web app).
# Usage: ./scripts/release.sh [vX.Y.Z] [--skip-nostr] [--skip-github] [--retag]
#
# Detects APP_VERSION in cacheflow.html, extracts the matching CHANGELOG entry,
...
APP_FILE="$REPO_ROOT/cacheflow.html"
...
# ── Extract version from cacheflow.html ─────────────────────────────────────
...
# Zapstore identity (matches pearcal-native)
...
DRAFT=$(mktemp /tmp/cacheflow-nostr-XXXXXX.txt)
printf '%s' "$NOTE_CONTENT" > "$DRAFT"
echo "    Opening note in ${EDITOR:-vi} for review..."
"${EDITOR:-vi}" "$DRAFT"
